...
class Strelka2(Module):
    ...
    def define_command(self):
        ...
        bailist             = self.get_argument("bam_idx")
        refgenome           = self.get_argument("ref")
        regionfile          = self.get_argument("bed_gz")
        strelka2            = self.get_argument("strelka2")
        nr_cpus             = self.get_argument("nr_cpus")

        print("List of bam files {0}".format(" ".join(bamlist)))
        print("List of bai files {0}".format(" ".join(bailist)))

        # Convert bam list into string
        bam_string = " ".join(["--bam {0}".format(_bam) for _bam in bamlist]) if isinstance(bamlist, list) else "--bam {0}".format(bamlist)

        cmd1 = "{0} {1} --referenceFasta {2} --callRegions={3} --runDir {4}".format(strelka2, bam_string, refgenome, regionfile, self.run_directory)
